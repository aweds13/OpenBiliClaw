...
| 渠道 caller 默认低档 reasoning | ✅ | `LLMService` 将 `discovery.*` / `recommendation.*` / `sources.*` / `yt_search.*` / `runtime.bilibili_extension_search.*` 以及三类轻量 eval caller 的未指定 effort 解析为 `"low"`（最低可移植档）；显式 caller 参数始终优先。不用 `""` 是为了兼容部分 OpenAI-compatible reasoning-first 模型直接拒绝“关闭思考”（例如返回 `code 1210: 不支持关闭思考，请使用 low、high 或 max`）。DeepSeek / OpenAI / Claude / Gemini 仍会把 `"low"` 映射到各自最低安全档；OpenRouter 因未持有 per-model mandatory metadata 而按 existing 逻辑处理。Soul / 画像与长场景继续使用 provider 的 `medium`（或用户配置值） |
